...
###--- data ---###
path_data_root = '../../../dataset/MIDI-BERT-Composer_512/composer-CP'
path_train_data = path_data_root + '/train_data_linear.npz'
path_dictionary = path_data_root + '/CP_new.pkl'

###--- answer data ---###
path_cls_answer_data = path_data_root + '/composer_train_ans.npy'
...
TICK_RESOL = BEAT_RESOL // 4        # position
...
class TransformerModel(nn.Module):
    def __init__(self, n_token, is_training=True):
        ...
        self.word_emb_composer = Embeddings(self.n_token[5], self.emb_sizes[5])
        # position == d_model
        ...
        self.proj_composer = nn.Linear(self.d_model, self.n_token[5])
    ...
    def train_step(self, x, target, loss_mask):
        h, y_composer = self.forward_hidden(x)
        ...
        y_composer = y_composer[:, ...].permute(0, 2, 1)
        ...
        loss_composer = self.compute_loss(
            y_composer, target[..., 5], loss_mask)

        return loss_bar, loss_position, loss_pitch, loss_duration, loss_type, loss_composer
    ...
    def forward_hidden(self, x, memory=None, is_training=True):
        ...
        emb_composer = self.word_emb_composer(x[..., 5])
        ...
        if is_training:
            ...
            y_composer = self.proj_composer(h)
            return h, y_composer
        else:
            ...
            # project type
            y_composer = self.proj_composer(h)
            return h, y_composer, memory

    def forward_output(self, h, y):
        '''
        for training
        '''
        tf_skip_composer = self.word_emb_composer(y[..., 5])

        # project other
        y_concat_type = torch.cat([h, tf_skip_composer], dim=-1)
    ...
    # for inference
    def froward_output_sampling(self, h, y_composer):
        '''
            for inference
        '''
        # decide composer-token
        cur_word_composer = F.gumbel_softmax(y_composer, tau=1, hard=True)
        cur_word_composer_ = torch.argmax(cur_word_composer, dim=-1)

        composer_word_t = cur_word_composer_.unsqueeze(0)

        tf_skip_composer = self.word_emb_composer(composer_word_t).squeeze(0)        # tf_skip_composer.size: torch.Size([1, 64])
        ...
        # sampling gen_cond                               sampling

        # collect
        next_arr = self.get_current_gumbel_softmax(y_bar, y_position, y_pitch, y_duration, y_type, cur_word_composer_)

        return next_arr

    def get_current_gumbel_softmax(self, y_bar, y_position, y_pitch, y_duration, y_type, next_composer):
        ...
        fake_nexts = torch.cat([torch.unsqueeze(next_bars, -1), torch.unsqueeze(next_positions, -1), torch.unsqueeze(next_pitchs, -1), torch.unsqueeze(next_durations, -1), torch.unsqueeze(next_types, -1), torch.unsqueeze(next_composer, -1)], dim=-1)
    ...
    def inference_from_scratch(self, dictionary, composer_kind, gen_len=512):
        event2word, word2event = dictionary

        init = np.array([
            [2, 16, 86, 64, 1, composer_kind],  # set style， ['Bar <PAD>', 'Position <PAD>', 'Pitch <PAD>', 'Duration <PAD>', 1, 0||1||2||3||4||5||6||7]          #
        ])
        ...
        with torch.no_grad():
            ...
            for step in range(init.shape[0]):
                ...
                h, y_composer, memory = self.forward_hidden(
                    input_, memory, is_training=False)

            print('------ generate ------')
            # for length
            for num_note in range(gen_len):
                # sample others
                next_arr = self.froward_output_sampling(h, y_composer)
...
def train():
    ...
    for epoch in range(n_epoch):
        ...
        for bidx in range(num_batch):  # num_batch
            ...
            # run
            loss_bar, loss_position, loss_pitch, loss_duration, loss_type, loss_composer \
                = net.train_step(batch_x, batch_y, batch_mask)
            nll_loss_arr = [loss_bar, loss_position, loss_pitch, loss_duration, loss_type, loss_composer]
            nll_loss = (loss_bar + loss_position + loss_pitch + loss_duration + loss_type + loss_composer) / 6
...
def generate():
    ...
    id_to_composer = ["Bethel", "Clayderman", "Einaudi", "Hancock", "Hillsong", "Hisaishi", "Ryuichi", "Yiruma"]
    for composer_id in range(8):
        print(composer_id, id_to_composer[composer_id])
        sidx = 0
        while sidx < per_class_num_songs:
            try:
                start_time = time.time()
                path_outfile = os.path.join(path_gendir, '{}_{}.mid'.format(id_to_composer[composer_id], str(sidx)))

                res = net.inference_from_scratch(dictionary, composer_kind=composer_id, gen_len=512)
...
def save_npy():
    ...
    for i in range(8):
        print('composer_kind: ', i)
        composer_arr = []
        for j in range(per_class_num_songs):         
            res = net.inference_from_scratch(dictionary, composer_kind=i, gen_len=512)
            res = res.tolist()

            composer_arr.append(res)
        composer_arr = np.array(composer_arr)

        print("composer_arr.shape: ", composer_arr.shape)
        arr_pros[i] = composer_arr

    process_kinds_data(arr_pros[0], arr_pros[1], arr_pros[2], arr_pros[3],
                       arr_pros[4], arr_pros[5], arr_pros[6], arr_pros[7],
                       save_path='./'+path_exp, file_name=path_exp)
