...
def write_some_midis(word2event, root='./emopia-CP', dataset='emopia_cp_train_new', answer='emopia_cp_train_ans'):
    data = np.load(os.path.join(root, f'{dataset}.npy'), allow_pickle=True)
    data_answer = np.load(os.path.join(root, f'{answer}.npy'))
    print(f'   {dataset}: {data.shape}')  # emopia_cp_train: (924, 513, 4)
    print(f'   {answer}: {data_answer.shape}')  # emopia_cp_train_ans: (924,)

    for data_id in range(924):
        cur_data = data[data_id]
        path_outfile = './test_changeData/' + str(data_id) + '.mid'
        write_midi(cur_data, path_outfile, word2event)

# 根据标签得到midis，分别统计
def label_midis(word2event, root='./emopia-CP', dataset='emopia_cp_train_new', answer='emopia_cp_train_ans'):
    data = np.load(os.path.join(root, f'{dataset}.npy'), allow_pickle=True)
    data_answer = np.load(os.path.join(root, f'{answer}.npy'))
    print(f'   {dataset}: {data.shape}')  # emopia_cp_train: (924, 513, 4)
    print(f'   {answer}: {data_answer.shape}')  # emopia_cp_train_ans: (924,)
...
if __name__ == '__main__':
    e2w, w2e = look_dict('./emopia-CP/CP_dict_new.pkl')
