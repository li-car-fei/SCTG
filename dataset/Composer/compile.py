...
TEST_AMOUNT = 50
WINDOW_SIZE = 512
GROUP_SIZE = 6                        # ['Bar', 'Position', 'Pitch', 'Duration', 'Type', 'Composer']

"""
MIDI-BERT 的数据处理好的都是 513 长度的，因此MAX_LEN不需要设置为怎么大了，后面都是补0而已
"""
...
if __name__ == '__main__':
    # paths
    # path_root = 'composer-CP'
    # data_indir = os.path.join(path_root, 'composer_cp_train_new.npy')

    # 泛化性能实验的数据预处理
    # path_root = 'generalization'
    # data_indir = os.path.join(path_root, 'composer_cp_finetune_with_Yiruma.npy')

    # 数据增强实验的数据预处理
    path_root = 'augmentation'
    data_indir = os.path.join(path_root, 'composer_cp_train.npy')

    # load dictionary
    path_dictionary = os.path.join('composer-CP', 'CP_new.pkl')
    ...
    for fidx in range(dataset.shape[0]):
        words = dataset[fidx]
        composer_label = words[0][-1]                               # 当前数据的 composer_label
        num_words = words.shape[0]
        # eos_arr = words[-1][None, ...]                            # 前一步骤在末尾添加的 eso event
        eos_arr = [[2, 16, 86, 64, 0, composer_label]]                       # 最后一个是composer，当只有头部标签时则是8即<Pad>
        print(' words_shape:', words.shape)     # words_shape: (513, 6)

        # arrange IO
        x = words[:-1].copy()
        y = words[1:].copy()
        seq_len = len(x)
        # print(' > x_shape:', x.shape)           # x_shape: (512, 6)
        # print(' > y_shape:', y.shape)           # y_shape: (512, 6)
    ...
    # print(' >   count:', )
    # print(' > x_final:', x_final.shape)                             # x_final: (1186, 513, 6)
    # print(' > y_final:', y_final.shape)                             # y_final: (1186, 513, 6)
    # print(' > mask_final:', mask_final.shape)                       # mask_final: (1186, 513)
    ...
    # 泛化性能评估实验数据预处理
    # path_train = os.path.join(path_root, 'finetune_data_Yiruma')
    # path_train += '.npz'
    # np.savez(
    #     path_train,
    #     x=x_final[train_idx],
    ...
    # 数据增强实验 数据预处理
    path_train = os.path.join(path_root, 'train_data_linear')
    path_train += '.npz'
    np.savez(
        path_train,
        x=x_final[train_idx],
